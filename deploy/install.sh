...
github_api_curl() {
    ...
    if [ -n "${UPDATE_GITHUB_TOKEN:-}" ]; then
        ...
        printf 'header = "Authorization: Bearer %s"\n' "$UPDATE_GITHUB_TOKEN" | env -u UPDATE_GITHUB_TOKEN -u GITHUB_TOKEN -u GH_TOKEN curl -q --globoff --config - "$@"
    else
        env -u UPDATE_GITHUB_TOKEN -u GITHUB_TOKEN -u GH_TOKEN curl -q --globoff "$@"
    fi
}
