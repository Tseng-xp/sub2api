...
func extractOpenAIUsageFromJSONBytes(body []byte) (OpenAIUsage, bool) {
	...
	for _, candidate := range candidates {
		if usage, ok := openAIUsageFromGJSON(gjson.GetBytes(body, candidate.usagePath)); ok {
			usage.ServiceTier = openAIResponseServiceTierFromBody(body)
...
func openAIResponseServiceTierFromBody(body []byte) string {
	for _, path := range []string{"service_tier", "response.service_tier", "data.service_tier", "data.response.service_tier"} {
		if tier := strings.TrimSpace(gjson.GetBytes(body, path).String()); tier != "" {
			return tier
		}
	}
	return ""
