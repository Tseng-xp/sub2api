...
// isTerminalUsageBillingError reports deterministic errors that cannot benefit
// from retrying. Transient repository/transaction failures remain retryable.
...
// applyUsageBillingWithRetry uses the repository's request-id idempotency to
// retry bounded transient failures without double charging.
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	// Prefer a candidate with explicit channel pricing before falling back to a
	// mapped/upstream alias that may only match built-in pricing.
...
// preferChannelPricedBillingModel selects the first candidate with explicit
// administrator channel pricing. If none match, the original primary model is
// retained so the existing fallback pricing path is unchanged.
