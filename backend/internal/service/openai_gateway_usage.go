...
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCost(
	...
	var lastErr error
	// An explicitly configured channel price must win over a generic fallback
	// price for an earlier alias in the candidate list.
	if len(billingModels) > 0 && billingModel != "" {
		for _, candidate := range billingModels {
			candidate = strings.TrimSpace(candidate)
			if candidate == "" || s.resolveOpenAIChannelPricing(ctx, candidate, apiKey) == nil {
				continue
			}
